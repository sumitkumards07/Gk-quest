...
import { collection, onSnapshot, addDoc, deleteDoc, doc, updateDoc, serverTimestamp, writeBatch, query, where, getDoc } from 'firebase/firestore';
...
export default function CampaignDetails() {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'questions'), where('campaignId', '==', campaignId));
    ...
    reader.onload = async (event) => {
      try {
        ...
        rawJson.forEach((quest) => {
          if (!quest.question || !quest.correctAnswer || !quest.optionA) return;
        ...
        if (count > 0) {
          await batch.commit();
          alert(`Successfully uploaded ${count} questions to this campaign!`);
        } else {
  ...
  const handleParseAndImport = async () => {
    try {
      ...
      for (let block of blocks) {
        ...
        if (!ansLine && cleanLines.length >= 6) ansLine = cleanLines[5];
      ...
      await batch.commit();
      alert(`Successfully imported ${validQuestions.length} questions into this campaign! ${parseErrors > 0 ? `(${parseErrors} blocks were skipped)` : ''}`);
  ...
  return (
    <>
      <input type="file" ref={fileInputRef} onChange={handleBulkUpload} style={{ display: 'none' }} accept=".json" />
      
      {/* Hero Header Area */}
      <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <button onClick={() => navigate('/campaigns')} className="p-2 hover:bg-surface-container rounded-lg text-primary transition-colors flex items-center gap-1 text-xs font-bold font-headline">
              <span className="material-symbols-outlined text-sm">arrow_back</span>
              Back to Campaigns
            </button>
          </div>
          <h1 className="text-4xl font-extrabold text-on-surface tracking-tight mb-2 font-headline italic">
            <span className="text-primary not-italic">{campaign?.title || 'Loading Campaign...'}</span>
          </h1>
          <p className="text-on-surface-variant max-w-lg font-medium">Manage questions specifically for the <span className="text-on-surface font-bold">"{campaign?.title}"</span> event. All questions added here are automatically linked to this schedule.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        {[
          { label: 'Campaign Questions', val: stats.total, color: 'text-primary' },
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-outline-variant/5">
              {loading ? (
                <tr>
                  <td colSpan="4" className="px-6 py-10 text-center text-on-surface-variant italic">Loading campaign questions...</td>
                </tr>
              ) : questions.length > 0 ? (
                questions.map((q) => (
                  <tr key={q.id} className="hover:bg-surface-container-low/30 transition-colors group">
                    <td className="px-6 py-5">
                      ...
                      <div className="flex gap-2">
                        <span className="text-[10px] bg-secondary-container/30 text-on-secondary-container px-2 py-0.5 rounded font-bold uppercase tracking-widest">Answer: {q.correctAnswer}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                ...
                <tr>
                  <td colSpan="4" className="px-6 py-10 text-center text-on-surface-variant italic">No questions in this campaign yet. Add one to get started!</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      {/* Manual Modals & Paste Modals (mostly same but without campaign dropdown) */}
      {showModal && (
          ...
          <div className="bg-surface-bright w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-8 py-6 bg-surface-container flex justify-between items-center">
              <div>
                ...
                <p className="text-xs font-bold text-primary uppercase tracking-widest mt-1">Campaign Linked: {campaign?.title}</p>
              </div>
              ...
              <form onSubmit={handleSave} className="space-y-6" id="question-form">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <select value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} className="w-full bg-surface-container-lowest border border-outline-variant/20 rounded-xl py-3 px-4 text-sm font-medium">
                  ...
                  <div className="space-y-2">
                    ...
                    <select value={formData.difficulty} onChange={(e) => setFormData({...formData, difficulty: e.target.value})} className="w-full bg-surface-container-lowest border border-outline-variant/20 rounded-xl py-3 px-4 text-sm font-medium">
                      <option>Easy</option>
                ...
                <div className="space-y-2">
                  ...
                  <textarea value={formData.question} onChange={(e) => setFormData({...formData, question: e.target.value})} required className="w-full bg-surface-container-lowest border border-outline-variant/20 rounded-xl py-3 px-4 text-sm font-medium min-h-[100px]" />
                </div>
                  ...
                  {['A', 'B', 'C', 'D'].map(opt => (
                    <div key={opt} className="flex items-center gap-3">
                      <div className="flex-1 relative">
                        ...
                        <input value={formData[`option${opt}`]} onChange={(e) => setFormData({...formData, [`option${opt}`]: e.target.value})} required className="w-full pl-10 pr-4 py-3 bg-surface-container-lowest border border-outline-variant/20 rounded-xl text-sm font-medium" type="text" />
                      </div>
                      <input checked={formData.correctAnswer === opt} onChange={() => setFormData({...formData, correctAnswer: opt})} className="w-5 h-5 text-secondary accent-secondary" name="correct" type="radio" />
                    </div>
                  ))}
                </div>
              </form>
        ...
        <div className="fixed inset-0 bg-inverse-surface/40 backdrop-blur-sm z-[60] flex items-center justify-center p-6">
          <div className="bg-surface-bright w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-8 py-6 bg-surface-container flex justify-between items-center border-b border-outline-variant/10">
              <div>
                <h2 className="text-2xl font-extrabold text-on-surface tracking-tight font-headline">Quick Import Content</h2>
                <p className="text-xs font-bold text-secondary uppercase tracking-widest mt-1">Linked to {campaign?.title}</p>
              </div>
            ...
            <div className="p-8 overflow-y-auto flex-1 flex flex-col gap-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <select id="paste-category" defaultValue="General" className="w-full bg-surface-container-lowest border border-outline-variant/20 rounded-xl py-3 px-4 text-sm font-medium">
                    <option>General</option>
                ...
                <div className="space-y-2">
                  ...
                  <select id="paste-difficulty" defaultValue="Easy" className="w-full bg-surface-container-lowest border border-outline-variant/20 rounded-xl py-3 px-4 text-sm font-medium">
                    <option>Easy</option>
                    <option>Intermediate</option>
                    <option>Expert</option>
                  </select>
              ...
              <div className="flex-1 min-h-[250px] flex flex-col">
                <textarea value={pasteText} onChange={(e) => setPasteText(e.target.value)} placeholder="Paste your questions text here..." className="w-full flex-1 bg-surface-container-lowest border border-outline-variant/20 rounded-xl py-4 px-5 text-sm font-medium focus:ring-2 focus:ring-primary/20 resize-none font-mono" />
              </div>
            </div>
            <div className="p-6 border-t border-outline-variant/10 flex justify-end gap-4 bg-surface-bright">
              <button onClick={() => setShowPasteModal(false)} className="px-6 py-3 text-sm font-bold text-on-surface-variant hover:bg-surface-container rounded-xl transition-colors" disabled={isBulkUploading}>Cancel</button>
              <button onClick={handleParseAndImport} disabled={isBulkUploading || !pasteText.trim()} className="px-8 py-3 bg-secondary text-secondary-container font-black rounded-xl shadow-lg shadow-secondary/20 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 flex items-center gap-2">
                <span className="material-symbols-outlined">{isBulkUploading ? 'hourglass_top' : 'bolt'}</span>
                {isBulkUploading ? 'Importing...' : 'Blast Content'}
              </button>
            </div>
          </div>
        </div>
